...
from volttrontesting.utils.platformwrapper import PlatformWrapper, get_platform_wrapper, PlatformWrapperError
...
def test_get_platform_wrapper_can_create(messagebus, ssl_auth):
    volttron_home = None
    with get_platform_wrapper(messagebus=messagebus, ssl_auth=ssl_auth) as p:
        volttron_home = p.volttron_home

        # Defaults to start = True
        assert p.is_running()
        assert p.volttron_home.startswith("/tmp/tmp")
        assert p.vip_address

        # We know if we got here that the platform has already been started, this says
        # don't add the services to auth file.
        with pytest.raises(PlatformWrapperError):
            p.startup_platform(vip_address=p.vip_address)
        assert p.is_running()
    ...
    assert volttron_home is not None

    if not os.environ.get('DEBUG'):
        # make sure clean up happens
        assert not Path(volttron_home).exists()
